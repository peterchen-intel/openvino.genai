...
ov::genai::WhisperPipeline::WhisperPipeline(const std::filesystem::path& models_path,
    ...
    if (device == "NPU") {
        ...
        if (!use_static_pipeline) {
            m_impl = std::make_unique<WhisperPipelineStatefulImpl>(models_path, device, properties_copy, core);
        } else {
            m_impl = std::make_unique<StaticWhisperPipeline>(models_path, properties_copy, core);
        }
    } else {
        m_impl = std::make_unique<WhisperPipelineStatefulImpl>(models_path, device, properties, core);
    }
    auto stop_time = std::chrono::steady_clock::now();
