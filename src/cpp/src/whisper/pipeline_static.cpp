...
WhisperPipeline::StaticWhisperPipeline::StaticWhisperPipeline(const std::filesystem::path& models_path,
    ...
    if (std::filesystem::exists(models_path / "openvino_decoder_with_past_model.xml") ) {
        decoder_model = m_core->read_model(models_path / "openvino_decoder_model.xml", {}, properties_copy);
        decoder_with_past_model = m_core->read_model(models_path / "openvino_decoder_with_past_model.xml", {}, properties_copy);
    } else {
        auto model = m_core->read_model(models_path / "openvino_decoder_model.xml", {}, properties_copy);
    ...
    compiled_model = m_core->compile_model(decoder_with_past_model, "NPU", properties_copy);
    ...
    compiled_model = m_core->compile_model(decoder_model, "NPU", properties_copy);
