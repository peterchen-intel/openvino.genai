...
std::unique_ptr<ov::genai::CircularBufferQueue<ov::InferRequest>> create_image_preprocessors(const std::shared_ptr<ov::Core>& core, const std::string& device) {
    using namespace ov;
    ...
    CompiledModel compiled = core->compile_model(model, device);
...
std::unique_ptr<ov::genai::CircularBufferQueue<ov::InferRequest>> create_separator_inserters(const std::shared_ptr<ov::Core>& core, const std::string& device) {
    using namespace ov;
    using namespace element;
    using namespace opset13;
    using namespace std;
...
m_image_preprocessors{create_image_preprocessors(core, device)},
m_separator_inserters{create_separator_inserters(core, device)} {
    auto compiled_model = core->compile_model(model_dir / "openvino_vision_projection_model.xml", device, {});
...
m_separator_inserters{create_separator_inserters(core, device)} {
    ...
    auto compiled_model = core->compile_model(vision_projection_model, vision_projection_weights, device, properties);
    m_ireq_queue_vision_projection = std::make_unique<CircularBufferQueue<ov::InferRequest>>(
        compiled_model.get_property(ov::optimal_number_of_infer_requests),
        [&compiled_model]() -> ov::InferRequest {
            return compiled_model.create_infer_request();
