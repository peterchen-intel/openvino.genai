...
VisionEncoderLLaVANextVideo::VisionEncoderLLaVANextVideo(
    ...
    const ov::AnyMap properties) : VisionEncoderLLaVANext(model_dir, device, core, properties),
    ...
    if (use_ov_vision_preprocess) {
        ...
        auto vision_encoder_model = core->read_model(model_dir / "openvino_vision_embeddings_model.xml");
    ...
    auto compiled_model = core->compile_model(model_dir / "openvino_multi_modal_projector_model.xml", device, {});
    m_ireq_queue_multi_modal_projector = std::make_unique<CircularBufferQueue<ov::InferRequest>>(
        compiled_model.get_property(ov::optimal_number_of_infer_requests),
        [&compiled_model]() -> ov::InferRequest {
            return compiled_model.create_infer_request();
        });
    compiled_model = core->compile_model(model_dir / "openvino_vision_resampler_model.xml", device, {});
...
VisionEncoderLLaVANextVideo::VisionEncoderLLaVANextVideo(
    ...
    const std::string& device,
    const std::shared_ptr<ov::Core>& core,
    const ov::AnyMap device_config) : VisionEncoderLLaVANext{models_map, config_dir_path, device, core, device_config},
        use_ov_vision_preprocess(can_use_ov_vision_preprocess()) {
    if (use_ov_vision_preprocess) {
        // Create integrated preprocessing + vision encoder model for image/video processing
        const auto& [vision_encoder_model, vision_encoder_weights] = utils::get_model_weights_pair(models_map, "vision_embeddings");
        auto vision_encoder_model_original = core->read_model(vision_encoder_model, vision_encoder_weights);
    ...
    auto compiled_model = core->compile_model(resampler_model, resampler_weights, device, device_config);
    ...
    compiled_model = core->compile_model(mm_projector_model, mm_projector_weights, device, device_config);
...
InputsEmbedderLLaVANextVideo::InputsEmbedderLLaVANextVideo(
    ...
    InputsEmbedderLLaVANext(vlm_config, model_dir, device, core, device_config) { }
...
InputsEmbedderLLaVANextVideo::InputsEmbedderLLaVANextVideo(
    ...
    const std::shared_ptr<ov::Core>& core,
    const ov::AnyMap device_config) :
    InputsEmbedderLLaVANext(vlm_config, models_map, tokenizer, config_dir_path, device, core, device_config) { }
