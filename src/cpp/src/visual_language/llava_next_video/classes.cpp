...
VisionEncoderLLaVANextVideo::VisionEncoderLLaVANextVideo(
    ...
    if (use_ov_vision_preprocess) {
        ...
        auto vision_encoder_model = ov::Core{}.read_model(model_dir / "openvino_vision_embeddings_model.xml");
        auto model = patch_preprocess_into_vision_encoder_model(vision_encoder_model, m_processor_config);
        auto compiled_model = ov::Core{}.compile_model(model, device, properties);
    ...
    auto compiled_model = ov::Core{}.compile_model(model_dir / "openvino_multi_modal_projector_model.xml", device, {});
    m_ireq_queue_multi_modal_projector = std::make_unique<CircularBufferQueue<ov::InferRequest>>(
        compiled_model.get_property(ov::optimal_number_of_infer_requests),
        [&compiled_model]() -> ov::InferRequest {
            return compiled_model.create_infer_request();
        });
    compiled_model = ov::Core{}.compile_model(model_dir / "openvino_vision_resampler_model.xml", device, {});
...
VisionEncoderLLaVANextVideo::VisionEncoderLLaVANextVideo(
    ...
    if (use_ov_vision_preprocess) {
        ...
        auto vision_encoder_model_original = ov::Core{}.read_model(vision_encoder_model, vision_encoder_weights);
        auto model = patch_preprocess_into_vision_encoder_model(vision_encoder_model_original, m_processor_config);
        auto compiled_model = ov::Core{}.compile_model(model, device, device_config);
        // Overwrite vision encoder queue with integrated model
        m_ireq_queue_vision_encoder = std::make_unique<CircularBufferQueue<ov::InferRequest>>(
            compiled_model.get_property(ov::optimal_number_of_infer_requests),
            [&compiled_model]() -> ov::InferRequest {
    ...
    auto compiled_model = ov::Core{}.compile_model(resampler_model, resampler_weights, device, device_config);
    ...
    compiled_model = ov::Core{}.compile_model(mm_projector_model, mm_projector_weights, device, device_config);
