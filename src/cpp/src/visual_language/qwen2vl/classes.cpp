...
VisionEncoderQwen2VL::VisionEncoderQwen2VL(const std::filesystem::path& model_dir,
    ...
    if (use_ov_vision_preprocess) {
        auto model_org = m_core->read_model(model_dir / "openvino_vision_embeddings_model.xml");
        m_ireq_queue_vision_encoder = create_vision_encoder_ireq(model_org, m_processor_config, device, properties, m_core);
    }
}
...
InputsEmbedderQwen2VL::InputsEmbedderQwen2VL(
    ...
    auto model = m_core->read_model(model_dir / "openvino_vision_embeddings_merger_model.xml");
...
InputsEmbedderQwen2VL::InputsEmbedderQwen2VL(
    ...
    auto compiled_model = m_core->compile_model(model,
        device,
        device_config
    );
