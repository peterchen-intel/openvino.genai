...
VisionEncoderMiniCPM::VisionEncoderMiniCPM(
        ...
        const ov::AnyMap properties) : VisionEncoder{model_dir, device, core, properties} {
    m_vlm_config = utils::from_config_json_if_exists<VLMConfig>(model_dir, "config.json");
    auto compiled_model = core->compile_model(model_dir / "openvino_resampler_model.xml", device, properties);
    ov::genai::utils::print_compiled_model_properties(compiled_model, "VLM resampler model", core);
...
VisionEncoderMiniCPM::VisionEncoderMiniCPM(
        ...
        const std::string& device,
        const std::shared_ptr<ov::Core>& core,
        const ov::AnyMap device_config) : VisionEncoder{models_map, config_dir_path, device, core, device_config} {
