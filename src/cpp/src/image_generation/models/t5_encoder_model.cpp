...
T5EncoderModel::T5EncoderModel(const std::filesystem::path& root_dir, const std::shared_ptr<ov::Core>& core) :
...
T5EncoderModel& T5EncoderModel::compile(const std::string& device, const ov::AnyMap& properties) {
    ...
    ov::genai::utils::print_compiled_model_properties(compiled_model, "T5 encoder model", m_core);
