...
class Text2VideoPipeline::LTXPipeline {
    ...
    std::shared_ptr<ov::Core> m_core = std::make_shared<ov::Core>();
...
public:
    ...
    LTXPipeline(const std::filesystem::path& root_dir,
        ...
        if (t5_text_encoder == "T5EncoderModel") {
            m_t5_text_encoder = std::make_shared<T5EncoderModel>(root_dir / "text_encoder", m_core);
        } else {
        ...
        if (vae == "AutoencoderKLLTXVideo") {
            m_vae = std::make_shared<AutoencoderKLLTXVideo>(root_dir / "vae_decoder", m_core);
        } else {
        ...
        if (transformer == "LTXVideoTransformer3DModel") {
            m_transformer = std::make_shared<LTXVideoTransformer3DModel>(root_dir / "transformer", m_core);
        } else {
    ...
    LTXPipeline(const std::filesystem::path& models_dir,
        ...
        : m_scheduler{cast_scheduler(Scheduler::from_config(models_dir / "scheduler/scheduler_config.json"))},
          m_t5_text_encoder{std::make_shared<T5EncoderModel>(models_dir / "text_encoder", device, properties, m_core)},
          m_transformer{std::make_shared<LTXVideoTransformer3DModel>(models_dir / "transformer", device, properties, m_core)},
          m_vae{std::make_shared<AutoencoderKLLTXVideo>(models_dir / "vae_decoder", device, properties, m_core)},
          m_generation_config{LTX_VIDEO_DEFAULT_CONFIG} {
    ...
    void rebuild_models() {
        m_t5_text_encoder = std::make_shared<T5EncoderModel>(m_models_dir / "text_encoder", m_core);
        m_transformer = std::make_shared<LTXVideoTransformer3DModel>(m_models_dir / "transformer", m_core);
        m_vae = std::make_shared<AutoencoderKLLTXVideo>(m_models_dir / "vae_decoder", m_core);
    }
