...
public:
    ...
    LTXPipeline(const std::filesystem::path& root_dir,
        ...
        if (t5_text_encoder == "T5EncoderModel") {
            m_t5_text_encoder = std::make_shared<T5EncoderModel>(root_dir / "text_encoder", m_ov_core);
        } else {
        ...
        if (vae == "AutoencoderKLLTXVideo") {
            m_vae = std::make_shared<AutoencoderKLLTXVideo>(root_dir / "vae_decoder", m_ov_core);
        } else {
        ...
        if (transformer == "LTXVideoTransformer3DModel") {
            m_transformer = std::make_shared<LTXVideoTransformer3DModel>(root_dir / "transformer", m_ov_core);
        } else {
    ...
    LTXPipeline(const std::filesystem::path& models_dir,
        ...
        : m_ov_core(core ? core : std::make_shared<ov::Core>()),
          ...
          m_t5_text_encoder{std::make_shared<T5EncoderModel>(models_dir / "text_encoder", device, properties, m_ov_core)},
          m_transformer{std::make_shared<LTXVideoTransformer3DModel>(models_dir / "transformer", device, properties, m_ov_core)},
          m_vae{std::make_shared<AutoencoderKLLTXVideo>(models_dir / "vae_decoder", device, properties, m_ov_core)},
          m_generation_config{LTX_VIDEO_DEFAULT_CONFIG} {
